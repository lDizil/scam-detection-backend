...
class ImageService:
    ...
    def _clean_ocr_text(self, text: str) -> str:
        """Убирает шум интерфейса, сохраняя важные данные"""

        # Убрать временные метки в квадратных скобках [HH:MM] или [HH:MM:SS]
        text = re.sub(r"\[\d{1,2}:\d{2}(?::\d{2})?\]", "", text)

        # Убрать префиксы номеров телефонов в начале сообщений (903 555-12-34:)
        # Но оставляем сами номера телефонов в тексте!
        text = re.sub(
            r"(?:^|\n)\d{3}\s\d{3}-\d{2}-\d{2}:\s*", " ", text, flags=re.MULTILINE
        )

        # Убрать повторяющиеся пробелы/переносы
